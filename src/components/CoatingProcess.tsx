...
export default function CoatingProcess() {
  ...
  const go = useCallback((i: number, byUser = false) => {
  ...
  useEffect(() => {
    ...
    [active, (active + 1) % COATING_STAGES.length].forEach(i => {
      ...
      if (!v) return;
      const want = `/coating/stage-${COATING_STAGES[i].n}.mp4`;
  ...
  return (
    <section className="coat" ref={sectionRef} aria-labelledby="coat-title">
      ...
      <div className="container coat-inner">
        <div className="coat-head">
          <p className="coat-eyebrow">Ceramic coating</p>
          <h2 className="font-display coat-display" id="coat-title">
            Four stages, <span className="coat-hl">one finish</span>
          </h2>
          <p className="coat-lede">
            A coating is only as good as what goes under it. Three of these four stages happen
            before the bottle is even opened.
          </p>
        </div>
